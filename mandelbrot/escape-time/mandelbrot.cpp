...
int main(int argc, char * argv[]) {
    if (argc != 6) {
        printf("Gebrauch: %s [xm] [ym] [d] [max_iteration] [scale] [shift]\n", argv[0]);
        return 1;
    }
    ...
    for (int Px = 0; Px < xsize; ++Px) {
    	for (int Py = 0; Py < ysize; ++Py) {
            ...
            mandelbrot::color_escape_time(&rgba[0], x0, y0, max_iteration, scale, shift);
