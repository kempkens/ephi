...
namespace mandelbrot {
	...
	void escape(double &x0, double &y0, unsigned &iteration, const unsigned max_iteration) {
		double t = x0 - 0.25;
	    double q = sq(t) + sq(y0);
	...
	unsigned escape_time(const double x0, const double y0, const unsigned max_iteration) {
	    double x = x0;
	    double y = y0;
	    unsigned iteration;
	...
	bool in_set(const double x0, const double y0, const unsigned max_iteration) {
		return escape_time(x0, y0, max_iteration) == max_iteration;
	}

	double distance_estimator(const double x0, const double y0, const unsigned max_iteration) {
	    double x = 0; // Re z
	    double y = 0; // Im z
	    double xsq = 0; // (Re z)^2
	    double ysq = 0; // (Im z)^2
	...
	void color_escape_time(double * c, const double x0, const double y0, const unsigned max_iteration, 
	...
	void color_distance_estimator(double * c, const double x0, const double y0, const unsigned max_iteration,
		const double k) {
